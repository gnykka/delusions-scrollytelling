document.addEventListener('DOMContentLoaded', () => {
  ...
  const onScroll = () => {
    ...
    const phase2 = texts[2].getBoundingClientRect().top - height * 0.1 <= 0;
    const phase3 = texts[3].getBoundingClientRect().top - height * 0.25 <= 0;
    const phase4 = texts[4].getBoundingClientRect().top - height * 0.25 <= 0;
    const phase5 = texts[5].getBoundingClientRect().top - height * 0.25 <= 0;
    ...
    svg.classList.toggle('phase-5', phase5 || bottom < height);
  }
...
// phase1 (фигура) - второй абзац
// третий-шестой абзацы: phase2 (голова), phase3 (змеи), phase4 (обезьяны), phase5 (рука)
